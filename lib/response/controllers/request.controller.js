...
module.exports = function(app) {
  ...
  app.controller("RequestController", ['$timeout', '$scope', '$rootScope', 'QuestionManager', 'ResponseManager', function($timeout, $scope, $rootScope, QuestionManager, ResponseManager) {
    ...
    $scope.captureContext = function(){
      console.log('works');
      //deactivate regular Codeon request button

      //activate context selection and a text box displaying along
      var contexts = {}
      contexts.editorViewMappings = {}

      atom.workspace.observeTextEditors(function (editor) {
        contexts.editorViewMappings[editor] = new TooltipView();
          ...
          sel.onDidChangeRange(function() {
            ...
            EditorView.addEventListener('mouseup', function(event) {
              ...
              if (!sel.isEmpty()) {
                ...
                contexts.editorViewMappings[editor].setParams({});

                contexts.editorViewMappings[editor].open();
              } else {
                contexts.editorViewMappings[editor].close();
              }
