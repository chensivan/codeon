      ...
      viewCodeDiff: function(answer){
			...
			showHelperCode: function(answers){
				...
				if(atom.workspace.getPanes().length>1){
					alert('Please close helper code first');
				}else{
					_.every(answers, function(ans){
						...
						if(ans.type=='inlinecode' || ans.type == 'annotation'){
							...
							ans.type=='inlinecode'? (helperCode = ans.value) : (helperCode = ans.helperCode);
							return false;
			...
			showCodeDiff: function(answers){
				...
				if(atom.workspace.getPanes().length>1){
					//move to the right panes
					// var moveToTheTab = atom.workspace.getPanes();
					// moveToTheTab[1].active = true;
					//do the thing
