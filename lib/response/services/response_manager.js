...
module.exports = function(app) {
  	...
  	var editor = {};
		...
		function addCodeDiff(response){
			...
			atom.workspace.open("/"+response.path).then(function(ed){
				...
				if(!lineMarkers.path.hasOwnProperty(response.path)){
					lineMarkers.path[response.path] = editor.getText();
				}

				lineMarkers.currentDev = lineMarkers.path[response.path];
				...
				if(mergeVersionOfCode.conflict){
					...
					//
					// var currentFirstPart  = currentDevArray.slice(0,requesterVersionStartIndex-1).join('\n')
    ...
    return rv = {
      ...
      viewCodeDiff: function(response){
					addCodeDiff(response)
      },
			viewMyCode: function(response){
				...
				if(!lineMarkers.path.hasOwnProperty(response.path)){
					...
					atom.workspace.open("/"+response.path).then(function(mergeTab){
						lineMarkers.path[response.path] = mergeTab.getText();
					});

				}else{ //if resposne path is in the linemarker path, load the value
					atom.workspace.open("/"+response.path).then(function(mergeTab){
						mergeTab.setText("");
						mergeTab.insertText(lineMarkers.path[response.path]);
					});
				}
				...
				//
				//
				// if(lineMarkers.hasOwnProperty('path') && lineMarkers.path!=response.path){
				// 	lineMarkers.path = response.path;
				// 	atom.workspace.open("/"+response.path).then(function(mergeTab){
			...
			viewHelperCode: function(response){
				if(!lineMarkers.hasOwnProperty('path')){
					lineMarkers.path = {};
				}
				...
				atom.workspace.open("/"+response.path).then(function(mergeTab){
					...
					mergeTab.setText("");
					mergeTab.insertText(response.response);
				});
			},
      mergeCode: function(){
