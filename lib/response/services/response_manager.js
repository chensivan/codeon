		...
		function addCodeDiff(response){
			...
			atom.workspace.open("/"+response.path).then(function(ed){
				...
				if(mergeVersionOfCode.conflict){
					...
					var file_extension = response.path.substr(response.path.lastIndexOf('.') + 1);
					atom.workspace.open("/"+response.path + ".merge."+file_extension).then(function(mergeTab){
						mergeTab.setText("");
						_.each(mergeVersionOfCode.result,function(line){
							console.log(line);
							mergeTab.insertText(line);
						});
					});
