...
module.exports = function(app) {
    ...
    var editorViewMappings = {};
  	var editor = {};
		...
		function markerDecoration(editor){
    ...
    return rv = {
      ...
      viewCodeDiff: function(answer){
			...
			finalMergeVersion: function(answer){
				removeMarker(tooltipMarkers);
			...
			showCodeDiff: function(answers){
				var request_path, helper_path, response_id,name,helperCode
				...
				_.every(answers, function(ans){
					...
					if(ans.type=='inlinecode'){
						var fileName = ans.path.split('/');
						request_path = ans.path;
						helper_path = "helper-"+fileName[fileName.length-1];
						response_id = ans.question_id;
						name = ans.name;
						helperCode = ans.value;
						return false;
					}

					if(ans.type == 'annotation'){
						request_path = ans.path;
						var fileName = ans.path.split('/');
						helper_path = "helper-"+fileName[fileName.length-1];
						response_id = ans.question_id;
						helperCode = ans.helperCode;
						return false;
				...
				atom.workspace.open(request_path).then(function(editor){
					editor.scrollToScreenPosition([0,0],{center:true});
					lineMarkers.path = {};
					lineMarkers.path[request_path] = editor.getText();
				}).then(function(){
					//split the tab
					...
					atom.workspace.open(helper_path)
						.then(function(mergeTab){
							lineMarkers.currentDev = lineMarkers.path[request_path];
							// get answer content
							lineMarkers.currentRes = helperCode;
							debugger;
							var currentDevArray = lineMarkers.currentDev.split("\n");
							var currentResArray = lineMarkers.currentRes.split("\n");
							...
							_.every(answers, function(answer){
      ...
      mergeCode: function(answer){
				...
				if(!lineMarkers.hasOwnProperty("currentRes")){
					addCodeDiff(answer);
				}
				...
				atom.workspace.open(answer.path).then(function(editor){
					editor.setText("");
					editor.insertText(mergeString);
				});

      },

			runOpenTabCode: function(){
				$rootScope.consoleResult;
        var currentOpenTabCode = atom.workspace.getActiveTextEditor().getText();
        ...
        console.log = function(message){
          ...
          document.querySelector('#runningEnvironment').appendChild(node);
        }
				debugger;
        allowUnsafeEval.Function(currentOpenTabCode);
			}
    };
